...
long OS::ChanIn (int idxChannel, char *buf, int &bufSize)
{
	...
	if(myChannel.isSet())
	{
		...
		{
			cerr<<" Could not transfer all data because  buffer size is too large!"<<endl;
			size=myChannel.getBufferSize();
		}

		//I arrived second at the rendez-vous, do the transfer
		memcpy(buf,myChannel.getBuffer(),myChannel.getBufferSize());
...
long OS::AltIn (int nChannels, int *channels, char *buf, int &bufSize, int &fromChannel)
{
	...
	{
		fromChannel= channels[i];
		Channel& myChannel= m_channels[fromChannel];
		

		if(myChannel.isSet())
		{	buf=myChannel.getBuffer();
			bufSize=myChannel.getBufferSize();
			return 0;
		}
		else {
		myChannel.set(buf, bufSize);
		sc_event_or_list blockingEvent(myChannel.m_event);
		TaskBlock(blockingEvent);
		blockingEvent|= myChannel.m_event;
		}
